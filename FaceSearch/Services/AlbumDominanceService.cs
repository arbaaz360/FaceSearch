...
using Infrastructure.Mongo.Models;                     // AlbumMongo, AlbumClusterMongo, ReviewMongo
...
namespace Application.Albums
{
    public sealed class AlbumDominanceService
    {
        ...
        /// and writes/updates the album document. Inserts a review when needed.
        /// </summary>
        public async Task<AlbumMongo> RecomputeAsync(string albumId, CancellationToken ct)
        {
            ...
            // ---- 5) Insert review items when applicable ------------------------------------
            if (ratio < AggregatorThreshold)
            {
                await _reviews.InsertAsync(new ReviewMongo
                {
